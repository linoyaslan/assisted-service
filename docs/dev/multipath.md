...
2. In SAN VM - "show virtual hardware details" -> Add Hardware -> Network -> select the isolated network you created in STEP-1 -> copy the MAC address (DON'T click finish yet)
3. In SNO-SAN VM - "show virtual hardware details" -> Add Hardware -> Network -> select the isolated network you created in STEP-1 -> copy the MAC address (DON'T click finish yet)
4. In your beaker machine - 
   - ```bash
     sudo virsh net-edit SAN-network
     ```
   - In the dhcp section add the following:

     ```xml
     <host mac='<SAN mac from point (1)>' name='SAN' ip='192.168.100.125'/>
     <host mac='<SNO-SAN mac from point (2)>' name='SNO-SAN' ip='192.168.100.126'/>
     ```

   - ```bash
     sudo virsh net-destroy SAN-network
      ```
   - ```bash
     sudo virsh net-start SAN-network
     ```
   - Check that the changes updated:
     ```bash
     sudo virsh net-dumpxml SAN-network
     ```
     example output:
      ```xml
        <network connections='2'>
          <name>SAN-network</name>
          <uuid>8cb8c0ea-a53c-4bc8-9724-89ad559b0604</uuid>
          <bridge name='virbr2' stp='on' delay='0'/>
          <mac address='52:54:00:1d:8f:ce'/>
          <domain name='SAN-network'/>
          <ip address='192.168.100.1' netmask='255.255.255.0'>
            <dhcp>
              <range start='192.168.100.128' end='192.168.100.254'/>
              <host mac='52:54:00:58:74:d1' name='SAN' ip='192.168.100.125'/>
              <host mac='52:54:00:09:e4:57' name='SNO-SAN' ip='192.168.100.126'/>
            </dhcp>
          </ip>
        </network>
     ```
5. Click "finish" in points 2-3. 
6. In SNO-SAN VM -
    ```bash
    iscsiadm --mode discovery --type sendtargets --portal 192.168.100.125:3260
    iscsiadm -m node -T iqn.2009-02.com.example:<iSCSI target you created in SAN> -p 192.168.100.125:3260 -l
    ```
6. Run `multipath -ll` to confirm that multipathing is active.
...
2. Create a VM that includes two different networks:
   ```bash
   deploy_nodes_with_networking MASTERS_COUNT=1
   ```
3. On the target side (beaker machine):
   ...
   - Create portal:
     ```bash
     targetcli /iscsi/iqn.2023-01.com.example:target01/tpg1/portals create <1st tearget IP>
     targetcli /iscsi/iqn.2023-01.com.example:target01/tpg1/portals create <2nd tearget IP>
     ```
